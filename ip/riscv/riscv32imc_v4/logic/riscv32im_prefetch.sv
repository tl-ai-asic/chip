...
module riscv32im_prefetch #(
  ...
  output logic [31:0] consume_predicted_pc_o,
  ...
  logic [31:0] rsp_predicted_pc_q [DEPTH];
  ...
  assign consume_predicted_pc_o = rsp_predicted_pc_q[rsp_head_q];
  ...
  always_ff @(posedge clk or negedge rst_n) begin
    ...
    if (!rst_n) begin
      ...
      for (reset_index = 0; reset_index < DEPTH; reset_index = reset_index + 1) begin
        ...
        rsp_predicted_pc_q[reset_index] <= 32'h0000_0004;
      ...
      branch_predicted_pc = 32'h0000_0000;
      ...
      if (imem_rsp_valid) begin
        if (flush_count_q != '0) begin
          ...
          branch_predict_taken = !imem_rsp_err &&
                                  (imem_rsp_rdata[6:0] == 7'b1100011) &&
                                  imem_rsp_rdata[31];
          branch_predicted_pc = branch_predict_taken ?
                                (req_pc_q[req_head_q] + imm_b(imem_rsp_rdata)) :
                                (req_pc_q[req_head_q] + 32'd4);

          rsp_insn_q[rsp_tail_q] <= imem_rsp_rdata;
          rsp_pc_q[rsp_tail_q] <= req_pc_q[req_head_q];
          rsp_predicted_pc_q[rsp_tail_q] <= branch_predicted_pc;
      ...
      if (branch_predict_taken) begin
        next_req_pc_next = branch_predicted_pc;
        flush_count_next = flush_count_next + req_count_next;
        req_head_next = '0;
        req_tail_next = '0;
        req_count_next = '0;
      end
