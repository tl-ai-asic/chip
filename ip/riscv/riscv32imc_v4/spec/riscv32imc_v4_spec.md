# riscv32imc_v4 Specification

## Overview

`riscv32imc_v4` is the carried-forward highest-throughput parallel version. It
builds on v3 by enabling parallel execution-engine issue/retirement paths,
...
in v2. v4 adds a conservative branch-prediction path for backward conditional
branches while keeping branch execution non-speculative.
...
- A four-entry prefetch queue overlaps sequential instruction fetch with
  backend execution and write-back. It predicts backward conditional branches
  taken when the branch instruction returns from instruction memory and stores
  the predicted next PC with the queued instruction.
...
- Correctly predicted conditional branches can retire through a guarded fast
  branch path when there are no outstanding older operations or busy registers.
  Mispredicted branches and all jumps/system controls use the original serial
  control path and redirect the prefetch queue on retirement.
...
```sh
make -C ip/riscv/riscv32imc_v4/tb
make -C ip/riscv/riscv32imc_v4/tb run-riscv-tests
make -C ip/riscv/riscv32imc_v4/tb run-ipc-tests
```
