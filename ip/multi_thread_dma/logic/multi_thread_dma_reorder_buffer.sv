...
module multi_thread_dma_reorder_buffer #(
  ...
  input  logic [THREAD_ID_WIDTH-1:0]   read_alloc_thread_id,
  ...
  output logic [THREAD_ID_WIDTH-1:0]   rd_rsp_thread_id,

  output logic                         wr_req_valid,
  input  logic                         wr_req_ready,
  output logic [ID_WIDTH-1:0]          wr_req_id,
  output logic [THREAD_ID_WIDTH-1:0]   wr_req_thread_id,
  ...
  output logic [THREAD_ID_WIDTH-1:0]   wr_rsp_thread_id
);
  ...
  logic [THREAD_ID_WIDTH-1:0] slot_thread_id_q [SLOT_COUNT];
  logic [THREAD_ID_WIDTH-1:0] slot_thread_id_d [SLOT_COUNT];
  ...
  assign rd_rsp_thread_id = slot_thread_id_q[rd_rsp_match_id];

  assign wr_req_valid = wr_req_slot_valid;
  assign wr_req_id = wr_req_slot_id;
  assign wr_req_thread_id = slot_thread_id_q[wr_req_slot_id];
  ...
  assign wr_rsp_thread_id = slot_thread_id_q[wr_rsp_match_id];
  ...
  always_comb begin
    ...
    for (int slot = 0; slot < SLOT_COUNT; slot++) begin
      ...
      slot_thread_id_d[slot] = slot_thread_id_q[slot];
    end
    ...
    if (clear) begin
      ...
      if (read_alloc_fire) begin
        ...
        slot_thread_id_d[read_alloc_id] = read_alloc_thread_id;
      end
  ...
  always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      ...
      for (int slot = 0; slot < SLOT_COUNT; slot++) begin
        ...
        slot_thread_id_q[slot] <= slot_thread_id_d[slot];
      end
    end
  end
...
`ifndef SYNTHESIS
  ...
  assert_read_alloc_uses_free_slot:
    ...
    else $error("multi_thread_dma ROB allocated an occupied slot");

  assert_read_alloc_thread_in_range:
    assert property (read_alloc_fire |-> int'(read_alloc_thread_id) < THREAD_COUNT)
    else $error("multi_thread_dma ROB allocated a slot for an invalid thread");
  ...
  assert_read_rsp_retires_read_slot:
    ...
    else $error("multi_thread_dma ROB did not retire read slot into write-pending state");
`endif
endmodule
