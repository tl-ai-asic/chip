...
ENABLE_IPC_TESTS = int(os.getenv("ENABLE_IPC_TESTS", "1"), 0) != 0
IPC_BENCHMARK_FILES = {
...
async def run_ipc_program(dut, name, data_response_latency=2, imem_response_latency=2):
    ...
    env = RiscvCoreTestbench(
        dut,
        data_response_latency=data_response_latency,
        imem_response_latency=imem_response_latency,
    )
    await env.start()
...
async def test_lsu_accepts_independent_memory_ops_while_pending(dut):
    env = RiscvCoreTestbench(dut, data_response_latency=12)
    await env.start()
